...
- The chapter tool group contains Search, Study Marks, Processing, and Study
  Data. Translate is no longer a primary workspace: there is no Translate
  toolbar or home control and no public Translation-workspace route.
...
- Language Study source-token cards place a compact Meaning control after Tags.
  It saves a personal meaning for the exact canonical `source_token` target;
  the value is not a Favorite or a classification tag.
- A pinned Strong's Word detail uses that same Meaning control only when active
  word context contains an exact canonical verse and source token. Transient
  reader hover and lexicon-only navigation do not expose an editable meaning.
- Selecting reader text exposes Favorite, Tags, Study, and Red letters actions.
  Favorite and Tags use one canonical `text_span` target.
...
|---|---|---|
| Word | Strong's/lexical detail, source word, morphology, and personal Meaning when an exact source token exists | Only when canonical word context exists |
...
The chapter Language Study control requires the internal interlinear capability
plus at least one tokenized verse in the current chapter. The verse `Int`
automation label represents the visibly rendered Language control and requires
tokens for that exact verse. A personal Meaning control also requires an exact
canonical verse and source-token target; a Strong's code alone is not
sufficient. Disabled controls use native `disabled`, `aria-disabled`, a reason
in `title`, and unavailable styling. The current scoped view uses
`aria-current="page"` and remains visually distinct.
...
- A personal meaning uses the same exact source-token scope but persists as a
  rendering record, not as a tag assertion or tag definition. Its compact
  editable badge remains distinct from tag badges.
...
- Bundled Translation renderings may remain informational Strong's detail, but
  they are distinct from a user's source-token-scoped personal Meaning.
...
## Personal meanings and portable legacy data

Personal meanings persist in `workspaceStore.token_renderings` with the
canonical schema-v2 `source_token` target and exact verse-plus-token-index
identity. They are separate from the tag store and are never synthesized as
custom tag definitions or assertion notes. Legacy token-rendering records that
lack canonical target metadata remain valid and editable; normalization adds
metadata without discarding the saved rendering.

The former Translation workspace has no primary editor. Its `verse_drafts`
records remain independent portable user data: import, export, merge, replace,
summary counting, and conflict handling preserve them without migrating them
into personal meanings. Existing `bibleapp:user-data` exports remain compatible.
...
- `tests/ui-contracts.mjs`: availability, panel transitions, control schema,
  corrected data scopes, token identity, and removal of the Translate workspace
  control.
...
- `tests/reader-ui-regressions.mjs`: source-level reader layout, panel-aware
  tooltip, concordance control, navigation-reset regressions, and absence of
  obsolete Translate/Strong's-context UI bridges.
